...
#define DW3000_PLL_XTAL_SETTLED_MASK        UINT32_C(0x40)
#define DW3000_PLL_LOCK_MASK                UINT32_C(0x02)
#define DW3000_PLL_CAL_DONE_MASK            UINT32_C(0x01)
...
static bool platform_is_valid(const dw3000_platform_t *platform)
{
  return (platform != NULL) &&
         ...
         ((platform->lock == NULL) == (platform->unlock == NULL));
}
    ...
    const dw3000_radio_config_t *config)
{
  ...
  sdk_config = (dwt_config_t){
    ...
    .pdoaMode = (dwt_pdoa_mode_e)config->pdoa_mode,
  };
  ...
  device->config_sdk_status = (int32_t)DWT_ERROR;
  device->config_transport_status = 0;
  device->config_pll_status = 0U;
  device->config_system_status_low = 0U;
  device->config_system_status_high = 0U;
  device->config_idle_rc = false;
  device->config_xtal_settled = false;
  device->config_pll_locked = false;
  device->config_pll_calibration_done = false;
  s_tx_pending = false;
  s_rx_pending = false;
  s_rx_data_ready = false;

  sdk_status = dwt_configure(&sdk_config);
  device->config_sdk_status = sdk_status;
  device->config_transport_status = s_transport_error;
  if (sdk_status != (int32_t)DWT_SUCCESS) {
    if (s_transport_error == 0) {
      device->config_pll_status = dwt_readpllstatus();
      device->config_system_status_low = dwt_readsysstatuslo();
      device->config_system_status_high = dwt_readsysstatushi();
      device->config_idle_rc = dwt_checkidlerc() != 0U;
      device->config_transport_status = s_transport_error;
      if (s_transport_error == 0) {
        device->config_xtal_settled =
            (device->config_pll_status &
             DW3000_PLL_XTAL_SETTLED_MASK) != 0U;
        device->config_pll_locked =
            (device->config_pll_status & DW3000_PLL_LOCK_MASK) != 0U;
        device->config_pll_calibration_done =
            (device->config_pll_status & DW3000_PLL_CAL_DONE_MASK) != 0U;
      }
    }
  ...
  dwt_configure_rf_port((dwt_rf_port_ctrl_e)config->rf_mode);
    ...
    const dw3000_platform_t *platform)
{
  ...
  *device = (dw3000_device_t){0};
  ...
  if (dw3000_qorvo_adapter_probe() != (int32_t)DWT_SUCCESS) {
    return (s_transport_error != 0)
               ? DW3000_STATUS_SPI_ERROR
               : DW3000_STATUS_UNSUPPORTED_DEVICE;
  }
    ...
    const dw3000_radio_config_t *config)
{
  ...
  sfd_valid =
      (config->sfd_type == DW3000_SFD_IEEE_4A) ||
      (config->sfd_type == DW3000_SFD_QORVO_8) ||
      (config->sfd_type == DW3000_SFD_QORVO_16) ||
  ...
  if (!channel_valid || !pac_valid || !rate_valid || !sfd_valid ||
      ...
      ((config->rf_mode < DW3000_RF_MODE_MANUAL_1) ||
       (config->rf_mode > DW3000_RF_MODE_AUTO_2_1)) ||
      (automatic_rf_mode !=
       (config->pdoa_mode != DW3000_PDOA_MODE_DISABLED)) ||
      ((config->sts_mode != DW3000_STS_OFF) &&
